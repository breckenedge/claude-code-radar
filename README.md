...
| [Cowork](https://claude.com/blog/cowork-research-preview) | Assess | Claude Code adapted for general-purpose computing. Dispatch (announced March 17, 2026) enables remote control from the Claude mobile app — trigger tasks from your phone while Claude runs locally. Cowork Projects (March 20, 2026) adds workspace organization (files, instructions, task context). Now a three-layer stack: Cowork (local agent) + Dispatch (mobile remote) + Channels (Telegram/Discord). Still early. |
| [Claude Code Channels](https://venturebeat.com/orchestration/anthropic-just-shipped-an-openclaw-killer-called-claude-code-channels) | Assess | First-party Telegram/Discord bridge for Claude Code (v2.1.80–81, March 2026). Configure with `/telegram:configure`; permission prompts relay to your phone. Removes the need for third-party bridge tools. Very new — no official docs page yet. |
...
| Blip | Ring | Notes |
|------|------|-------|
| [MCP (Model Context Protocol)](https://modelcontextprotocol.io) | Adopt | Open standard by Anthropic. Widely adopted across editors and platforms. MCP Elicitation (v2.1.76, March 2026) enables servers to request structured user input mid-task via an interactive dialog. 2026 roadmap priorities: stateless Streamable HTTP transport, enterprise auth/audit, Linux Foundation governance, and MCP Apps extension for interactive UI components in conversations. Tool Annotations (March 2026): five active SEPs are refining the risk vocabulary (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`) — a Tool Annotations Interest Group is forming; see [MCP blog post](https://blog.modelcontextprotocol.io/posts/2026-03-16-tool-annotations/) for the unresolved tension between hints and client trust. |
...
| Headless / CI Mode | Trial | Running Claude Code non-interactively via `claude -p`. Growing adoption but auth is a pain point — `setup-token` scoping is [buggy](https://github.com/anthropics/claude-code/issues/23703), most teams fall back to API keys in CI. New `--bare` flag (v2.1.81) is the right choice for scripted CI calls: skips hooks, LSP, plugin sync, and skill directory walks; requires `ANTHROPIC_API_KEY` or `apiKeyHelper`; auto-memory fully disabled. |
| Prompt Caching & Context Management | Adopt | Essential. Built into the platform. Every serious user manages context and caching. |
| Human-in-the-loop Agent Review | Trial | Reviewing agent-generated code before committing. Most teams already do this informally. Documented cases of Claude Code continuing after explicit stop commands (March 2026, widely discussed) reinforce why this pattern matters — never assume the agent stopped when you asked. |
...
| Voice Mode | Assess | Native `/voice` command (hold Space to record, Whisper STT) shipped in v2.1.71 (Mar 2026) with gradual rollout to 5% of users. Has known bugs in early builds — transcription pipeline disabled in some releases. Cloudflare WebSocket detection bug fixed in v2.1.80; audio recovery also improved. Community MCP alternatives (e.g., VoiceMode MCP) are more stable today. Too early for daily use but stabilizing. |
...
| Blip | Ring | Notes |
|------|------|-------|
| [VS Code + Claude Code Extension](https://code.claude.com/docs/en/vs-code) | Trial | Multi-agent support as of Feb 2026. Promising but buggy — extension host crashes, memory issues, rough terminal-to-UI handoff. v2.1.79 adds `/remote-control` to bridge local sessions to claude.ai/code. |
...
| [GitHub Actions + Claude Code](https://github.com/anthropics/claude-code-action) | Trial | CI/CD integration for automated code review and PR generation. Reached v1.0 GA (Feb 2026) with breaking changes from beta; migration guide available. Simplified configuration, automatic mode detection, structured JSON outputs. v1.0.77 (March 2026) adds subprocess environment scrubbing — strips Anthropic/cloud-provider credentials from subprocess environments in untrusted-input workflows (auto-enabled when `allowed_non_write_users` is set). Node.js 24 migration required before June 2, 2026 (GitHub Actions deadline). |
...
| [Claude Max / Pro / Team Subscriptions](https://claude.com/pricing) | Adopt | The economics of agent coding. Individual devs and small teams pay through Max or Pro plans. As of Jan 2026, Claude Code is included with every Team plan standard seat ($20/month). As of March 2026: 1M token context (Opus 4.6 and Sonnet 4.6) is now at standard API pricing with no long-context premium — Opus 4.6 at $5/$25/MTok input/output; Sonnet 4.6 at $3/$15/MTok; Haiku 4.5 at $1/$5/MTok. |
| [Claude in Chrome](https://code.claude.com/docs/en/chrome) | Assess | Browser automation from Claude Code via the Claude Chrome extension (Beta). Claude can navigate pages, click, fill forms, read console logs, and capture screenshots directly from the CLI or VS Code. Works with any site you're already logged into. Paid-only; supports Chrome and Edge. Interesting for build-test-debug loops. Note: v1.0.63 (March 20, 2026) introduced a connection regression — extension shows "Claude in Chrome is not connected" after update; track [#36910](https://github.com/anthropics/claude-code/issues/36910). |
...
This is opinionated and point-in-time (last updated: 2026-03-23). Blips move between rings as the ecosystem evolves.
...
20. [TechCrunch: Anthropic launches code review tool (March 9, 2026)](https://techcrunch.com/2026/03/09/anthropic-launches-code-review-tool-to-check-flood-of-ai-generated-code/) — Claude Code Review launch details, pricing, and availability informing new Assess blip
21. [VentureBeat: Anthropic rolls out Code Review for Claude Code (March 9, 2026)](https://venturebeat.com/technology/anthropic-rolls-out-code-review-for-claude-code-as-it-sues-over-pentagon) — Additional Code Review launch context
22. [MCP Blog: 2026 MCP Roadmap](https://blog.modelcontextprotocol.io/posts/2026-mcp-roadmap/) — MCP 2026 priorities informing MCP blip notes update
23. [Claude Code releases v2.1.72–v2.1.76 (March 10–14, 2026)](https://github.com/anthropics/claude-code/releases) — ExitWorktree tool, worktree.sparsePaths, PostCompact/Elicitation hooks, MCP Elicitation, 1M context default informing multiple blip updates
24. [Claude Code v2.1.71 release](https://github.com/anthropics/claude-code/releases/tag/v2.1.71) — Source for /loop command, cron scheduling, and voice mode rollout details
25. [Claude Code scheduled tasks docs](https://code.claude.com/docs/en/scheduled-tasks) — Official documentation for /loop and CronCreate/CronList/CronDelete tools
26. [Anthropic: Claude Code and new admin controls for business plans](https://www.anthropic.com/news/claude-code-on-team-and-enterprise) — Jan 2026 announcement that Claude Code is included with every Team plan standard seat
...
30. [MCP Blog: Tool Annotations as Risk Vocabulary (March 16, 2026)](https://blog.modelcontextprotocol.io/posts/2026-03-16-tool-annotations/) — Analysis of MCP tool annotation hints and five active SEPs; basis for MCP blip update on Tool Annotations Interest Group
31. [VentureBeat: Anthropic ships Claude Code Channels (March 2026)](https://venturebeat.com/orchestration/anthropic-just-shipped-an-openclaw-killer-called-claude-code-channels) — Claude Code Channels (Telegram/Discord bridge) launch; basis for new Channels Assess blip
32. [COEY: Anthropic Dispatch turns Claude into always-on coworker (March 17, 2026)](https://coey.com/resources/blog/2026/03/17/anthropic-dispatch-turns-claude-into-your-always-on-creative-coworker/) — Dispatch mobile remote control for Cowork; informing Cowork blip update
33. [Cybernews: Claude Code disregarding developer commands (March 2026)](https://cybernews.com/security/claude-code-disregarding-developers-commands/) — Documented cases of Claude Code continuing after explicit stop; informing Human-in-the-loop note
34. [GitHub Releases: anthropics/claude-code-action v1.0.77](https://github.com/anthropics/claude-code-action/releases) — Subprocess environment credential scrubbing; informing GitHub Actions blip update
35. [GitHub Releases: anthropics/claude-agent-sdk-python v0.1.49–0.1.50](https://github.com/anthropics/claude-agent-sdk-python/releases) — AgentDefinition skills/memory/mcpServers, session tagging; informing Agent SDK blip update
36. [1M token context at standard API pricing (March 2026)](https://thenewstack.io/claude-million-token-pricing/) — No long-context premium for Opus 4.6 and Sonnet 4.6; informing Claude Max/Pro/Team blip update
