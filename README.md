...
| Blip | Ring | Notes |
|------|------|-------|
| [MCP (Model Context Protocol)](https://modelcontextprotocol.io) | Adopt | Open standard by Anthropic. Widely adopted across editors and platforms. MCP Elicitation (v2.1.76, March 2026) enables servers to request structured user input mid-task via an interactive dialog. 2026 roadmap priorities: stateless Streamable HTTP transport, enterprise auth/audit, Linux Foundation governance, and MCP Apps extension for interactive UI components in conversations. Tool descriptions and server instructions now capped at 2KB (v2.1.84) to prevent OpenAPI-generated servers from bloating context. Large result persistence: servers can annotate results with `_meta["anthropic/maxResultSizeChars"]` (up to 500K chars, v2.1.91) to bypass silent truncation — critical for DB schemas and large tool outputs. OAuth updated to RFC 9728 Protected Resource Metadata discovery (v2.1.85). |
...
| [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk-python) | Trial | Same engine as Claude Code, exposed as a Python/TypeScript library. For building custom agents programmatically. Microsoft Agent Framework integration. Notable recent additions: `fork_session()` and `delete_session()` (Python v0.1.51); `startup()` pre-warm method in the TypeScript SDK (v0.2.89) makes first query ~20x faster by pre-connecting before issuing a prompt; `get_context_usage()` / `getContextUsage()` available in both SDKs. |
...
| [Hooks](https://code.claude.com/docs/en/hooks) | Adopt | Stable, well-documented. Standard for CI/build integration. Now supports four handler types: command (shell), HTTP (POST JSON to a URL), prompt (LLM-evaluated decision), and agent (subagent with tools). HTTP hooks enable webhook integrations without shell scripts. Recent additions (v2.1.76): `PostCompact` fires after context compaction; `Elicitation`/`ElicitationResult` hooks can intercept and override MCP server input requests. New events (v2.1.77–v2.1.89): `StopFailure` (fires on API error/rate limit), `CwdChanged`/`FileChanged` (reactive env management, e.g. direnv), `PermissionDenied` (auto mode classifier denials — return `{retry: true}` to retry), `TaskCreated`. Conditional `if` field (v2.1.85) filters when a hook fires using permission rule syntax (e.g. `Bash(git *)`), reducing unnecessary process spawning. Note: hooks in project config files were a supply chain RCE vector (CVE-2025-59536, patched in v1.0.111+). |
...
| Headless / CI Mode | Trial | Running Claude Code non-interactively via `claude -p`. Growing adoption but auth is a pain point — `setup-token` scoping is [buggy](https://github.com/anthropics/claude-code/issues/23703), most teams fall back to API keys in CI. `--bare` flag (v2.1.81) skips hooks, LSP, plugin sync, and skill walks for faster scripted execution. `MCP_CONNECTION_NONBLOCKING=true` (v2.1.89) skips MCP connection wait entirely in `-p` mode. `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1` (v2.1.83) strips credentials from subprocesses — important for prompt-injection hardening. |
...
| Claude Code Channels | Assess | Research preview (`--channels` flag, v2.1.80). Allows MCP servers to push messages into a running Claude Code session from external messaging apps (Telegram, Discord, iMessage). The permission relay mode (v2.1.81) can forward tool approval prompts to your phone. Interesting for async and remote-control workflows — no stable API yet, very early. |
...
| [GitHub Actions + Claude Code](https://github.com/anthropics/claude-code-action) | Trial | CI/CD integration for automated code review and PR generation. Reached v1.0 GA (Feb 2026) with breaking changes from beta; migration guide available. Simplified configuration, automatic mode detection, structured JSON outputs. Worth adopting for greenfield CI workflows now. Security hardening (Mar 2026): `.claude/` and `.mcp.json` now restored from the PR base branch before the CLI runs (v1.0.74), preventing supply-chain tampering via malicious PR branches. Subprocess env scrubbing (v1.0.77): Anthropic/cloud credentials and OIDC tokens are stripped from Claude-spawned subprocesses by default when `allowed_non_write_users` is configured, blocking prompt-injection exfiltration via shell expansion. |
...
| PowerShell Tool (Windows) | Assess | Opt-in preview since v2.1.84. Enables Claude to run PowerShell commands natively on Windows without requiring WSL. Multiple security hardening fixes shipped across v2.1.85, v2.1.89, and v2.1.90 (argument-splitting, background-job bypass, TOCTOU). Enable via settings. Too early to recommend broadly; worth watching for Windows-native workflows. |
...
This is opinionated and point-in-time (last updated: 2026-04-06). Blips move between rings as the ecosystem evolves.
...
30. [Claude Code releases v2.1.77–v2.1.92](https://github.com/anthropics/claude-code/releases) — April 2026 release notes informing new blips (Channels, PowerShell Tool) and updates to Hooks, MCP, Headless/CI Mode, and Agent SDK blips
31. [claude-code-action releases](https://github.com/anthropics/claude-code-action/releases) — v1.0.74 and v1.0.77 security hardening (config restoration, subprocess env scrubbing) informing GitHub Actions blip update
32. [Claude Agent SDK Python releases](https://github.com/anthropics/claude-agent-sdk-python/releases) — v0.1.51 (`fork_session`) and TypeScript SDK v0.2.89 (`startup()` pre-warm) informing Agent SDK blip update
