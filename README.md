...
| Blip | Ring | Notes |
|------|------|-------|
| [Claude Code](https://github.com/anthropics/claude-code) | Adopt | The reference agentic coding CLI. Largest ecosystem, most active development. npm install deprecated; native installer is now recommended — run `claude install`. Opus 4.6 output tokens raised to 64k default / 128k max (v2.1.77). Response text now streams line-by-line (v2.1.78). `--bare` flag (v2.1.81) for clean scripted `-p` calls without hooks, LSP, or plugin overhead. `/fork` renamed to `/branch` (v2.1.77, alias preserved). |
...
| [Cowork](https://claude.com/blog/cowork-research-preview) | Assess | Claude Code adapted for general-purpose computing. Cowork Dispatch launched March 17/22 (phone-to-desktop remote control for Max/Pro users — execution stays local on desktop). Computer use added ~March 23 for Pro/Max: Claude can interact with desktop apps and dev tools directly. Moving fast but still experimental. |
...
| Blip | Ring | Notes |
|------|------|-------|
| [MCP (Model Context Protocol)](https://modelcontextprotocol.io) | Adopt | Open standard by Anthropic. Widely adopted across editors and platforms. MCP Elicitation (v2.1.76, March 2026) enables servers to request structured user input mid-task via an interactive dialog. 2026 roadmap priorities: stateless Streamable HTTP transport, enterprise auth/audit, Linux Foundation governance, and MCP Apps extension for interactive UI components in conversations. RFC 9728 Protected Resource Metadata OAuth discovery and SEP-991 Client ID Metadata Document (CIMD) support added (v2.1.81/2.1.85) for enterprise auth flows. MCP tool descriptions and server instructions capped at 2KB (v2.1.84). Local config wins over claude.ai connector when the same server appears in both (v2.1.84). |
...
| [Hooks](https://code.claude.com/docs/en/hooks) | Adopt | Stable, well-documented. Standard for CI/build integration. Now supports four handler types: command (shell), HTTP (POST JSON to a URL), prompt (LLM-evaluated decision), and agent (subagent with tools). HTTP hooks enable webhook integrations without shell scripts. Recent additions (v2.1.76): `PostCompact` fires after context compaction; `Elicitation`/`ElicitationResult` hooks can intercept and override MCP server input requests. New hook events (March 2026): `StopFailure` fires when a turn ends on API error/rate limit (v2.1.78); `CwdChanged`/`FileChanged` enable reactive environment management like direnv (v2.1.83); `TaskCreated` fires on task spawn (v2.1.84). Conditional `if` field (v2.1.85) lets hooks fire only when matching permission rule syntax (e.g., `Bash(git *)`). `PreToolUse` hooks can now satisfy `AskUserQuestion` calls by returning `updatedInput` (v2.1.85). Note: hooks in project config files were a supply chain RCE vector (CVE-2025-59536, patched in v1.0.111+). |
...
| Headless / CI Mode | Trial | Running Claude Code non-interactively via `claude -p`. Growing adoption but auth is a pain point — `setup-token` scoping is [buggy](https://github.com/anthropics/claude-code/issues/23703), most teams fall back to API keys in CI. `--bare` flag (v2.1.81) for fully scripted calls — skips hooks, LSP, plugin sync, and OAuth/keychain; ideal for pure programmatic use without plugin overhead. |
...
| [Scheduled Tasks (/loop)](https://code.claude.com/docs/en/scheduled-tasks) | Assess | `/loop` command (v2.1.71, Mar 2026) runs prompts on a cron schedule within a session — turns Claude Code into a background worker. Session-scoped (stops when terminal closes), max 50 tasks per session, 3-day auto-expiry. For persistent scheduling, pair with GitHub Actions (`schedule:` trigger) or headless `claude -p`. Opens up new autonomous workflow patterns; too early to recommend broadly. Note: `TaskOutput` tool deprecated (v2.1.83) — read task output via `Read` on the task's output file path instead. |
...
| [GitHub Actions + Claude Code](https://github.com/anthropics/claude-code-action) | Trial | CI/CD integration for automated code review and PR generation. Reached v1.0 GA (Feb 2026) with breaking changes from beta; migration guide available. Simplified configuration, automatic mode detection, structured JSON outputs. v1.0.77: automatic subprocess env scrubbing for untrusted-input workflows — when `allowed_non_write_users` is set, strips API keys, cloud credentials, and OIDC tokens from agent subprocesses (`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`). Critical for public-repo workflows. Worth adopting for greenfield CI workflows now. |
...
This is opinionated and point-in-time (last updated: 2026-03-30). Blips move between rings as the ecosystem evolves.
...
30. [Claude Code releases v2.1.77–v2.1.87 (March 17–29, 2026)](https://github.com/anthropics/claude-code/releases) — Release notes informing updates to Claude Code, Cowork, Hooks, MCP, Git Worktree, Headless/CI, and Scheduled Tasks blips
31. [Claude Agent SDK v0.1.49–v0.1.52 (March 2026)](https://github.com/anthropics/claude-agent-sdk-python/releases) — Release notes informing Claude Agent SDK blip update (AgentDefinition expansion, session fork/delete, context usage API)
32. [claude-code-action v1.0.77 (March 23, 2026)](https://github.com/anthropics/claude-code-action/releases) — Automatic subprocess environment scrubbing for untrusted-input workflows, informing GitHub Actions blip update
