...
| [Claude Code Security](https://www.anthropic.com/news/claude-code-security) | Assess | AI-driven vulnerability scanning integrated with Claude Code. Moved from research preview to public beta for Enterprise customers (May 2026); powered by Opus 4.7. Supports scheduled and targeted scans, triage tracking, and fix proposals. Found 500+ vulnerabilities in production OSS. Enterprise-only for now. |
...
| [Claude Code Plugin System](https://github.com/anthropics/claude-code/blob/main/plugins/README.md) | Trial | 9,000+ plugins across marketplaces. API still evolving. Plugins can now be loaded from `.zip` archives or URLs via `--plugin-dir`/`--plugin-url` flags (v2.1.128+), easing distribution outside marketplaces. |
...
| [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk-python) | Trial | Same engine as Claude Code, exposed as a Python/TypeScript library. For building custom agents programmatically. Microsoft Agent Framework integration. v0.1.74+: hook event streaming (`include_hook_events`), `defer` hook decisions, `strict_mcp_config`, `xhigh` effort level for Opus 4.7, auto-terminate CLI subprocesses on parent exit. Note: `'Skill'` in `allowed_tools` is deprecated — use the `skills` option instead. |
| [Claude-Plugins.dev](https://claude-plugins.dev) | Assess | Alternative community marketplace. Smaller, less vetted. |
| [AWS MCP Server](https://aws.amazon.com/about-aws/whats-new/2026/05/aws-mcp-server/) | Trial | AWS-managed MCP server giving Claude Code agents direct, auditable access to any AWS API. GA May 2026. Provides a unified tool for any AWS API call, sandboxed Python execution against AWS services, and on-demand Agent Skills discovery — no credentials needed in the Claude session. Available in US East (N. Virginia) and EU (Frankfurt). |
...
| Model Switching Strategies | Trial | Haiku for quick passes, Sonnet for daily work, Opus for hard problems. Mixing models to balance cost and quality. Opus 4.7 adds an `xhigh` effort level for maximum reasoning depth on the hardest problems. |
...
| [Claude Max / Pro / Team Subscriptions](https://claude.com/pricing) | Adopt | The economics of agent coding. Individual devs and small teams pay through Max or Pro plans. As of Jan 2026, Claude Code is included with every Team plan standard seat ($20/month). As of March 2026: Opus 4.6's 1M token context window is now active by default for Max, Team, and Enterprise plans. As of May 2026: rate limits doubled for all paid plans (5-hour cap doubled, peak-hour throttling removed for Pro/Max); Opus 4.7 is the default model for Max and Team Premium within Claude Code. |
...
This is opinionated and point-in-time (last updated: 2026-05-11). Blips move between rings as the ecosystem evolves.
...
30. [Code with Claude SF 2026 recap (Blake Crosley)](https://blakecrosley.com/blog/code-with-claude-sf-2026-recap) — May 6 conference announcements: rate limits doubled, Managed Agents multiagent/Outcomes/Dreaming public beta, Vercept acquisition
31. [Claude Code releases v2.1.128–v2.1.138](https://github.com/anthropics/claude-code/releases) — May 2026 releases informing plugin ZIP/URL loading, worktree.baseRef, effort level in hooks, hard_deny auto mode, and Agent SDK deprecations
32. [Claude Agent SDK releases v0.1.73–v0.1.80](https://github.com/anthropics/claude-agent-sdk-python/releases) — May 2026 releases informing hook event streaming, xhigh effort, auto-terminate, skills option deprecation
33. [AWS MCP Server GA announcement](https://aws.amazon.com/about-aws/whats-new/2026/05/aws-mcp-server/) — Basis for new AWS MCP Server Trial blip
34. [What's New — Claude Code Docs (Week 19, May 4–8)](https://code.claude.com/docs/en/whats-new) — Official weekly digest confirming top features: plugin ZIP/URL, worktree.baseRef, hard_deny, effort hooks
